...
const KategoriPenanganan = require("../models/kategoriPenanganan");
...
const createKategori = async (req, res) => {
	...
	// PAYLOAD
	const { nama_penanganan } = req.body;
	// VALIDASI
		...
		config.log = [
			{
				...
				data: {
					tanggal: new Date(),
					kategori: "Menambahkan kategori penanganan",
					keterangan: nama_penanganan,
...
const editKategori = async (req, res) => {
    ...
    let check = [
		{
			model: KategoriPenanganan,
			whereCondition: {id_kategori_pgn: kode},
			title: "Kategori Penanganan",
			check: "isAvailable",
		},
	];
	...
	const transaction = await sequelize.transaction();
	try {
		const kategori = await KategoriPenanganan.findByPk(kode)
		config.data = {
			id_kategori_pgn: kode,
			nama_penanganan: nama_penanganan ?? kategori.nama_penanganan
		}
		config.log = [
			{
				model: LogUser,
				data: {
					tanggal: new Date(),
					kategori: "Mengubah kategori penanganan",
					keterangan: nama_penanganan,
					username: req.session.user,
				}
			}
		...
		res.status(201).json({
			status: "success",
			code: 201,
			message: ["Kategori penanganan berhasil diupdate"],
		});
	} catch (error) {
		await transaction.rollback();
		res.status(500).json({
...
const deleteKategori = async (req, res) => {
	...
	const transaction = await sequelize.transaction();
	try {
		config.data = { id_kategori_pgn: kode }
		config.log = [
			{
				model: LogUser,
				data: {
					tanggal: new Date(),
					kategori: "Menghapus kategori penanganan",
					keterangan: kode,
					username: req.session.user,
				}
			}
